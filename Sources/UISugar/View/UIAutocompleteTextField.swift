...
open class UIAutocompleteTextField: UITextField {
   ...
   /// Set your custom visual theme, or just choose between pre-defined UIAutocompleteTextFieldTheme.lightTheme() and UIAutocompleteTextFieldTheme.darkTheme() themes
   open var theme = UIAutocompleteTextFieldTheme.lightTheme() {
      didSet {
         ...
         if let hightlightedFont = self.highlightAttributes[.font] as? UIFont {
            self.highlightAttributes[.font] = hightlightedFont.withSize(self.font?.pointSize ?? 14)
         }
      }
   }
   ...
   /// Set an array of UIAutocompleteTextFieldItem's to be used for suggestions
   open func filterItems(_ items: [UIAutocompleteTextFieldItem]) {
      filterDataSource = items
   }
   
   /// Set an array of strings to be used for suggestions
   open func filterStrings(_ strings: [String]) {
      filterItems(strings.compactMap({ UIAutocompleteTextFieldItem(title: $0) }))
   }
   
   /// Closure to handle when the user pick an item
   open var itemSelectionHandler: UIAutocompleteTextFieldItemHandler?
   
   /// Closure to handle when text did change.
   open var textDidChangeHandler: UIAutocompleteTextDidChangeHandler?
   ...
   open var highlightAttributes: [NSAttributedString.Key: AnyObject] = [
   ...
   open var tableBottomMargin: CGFloat = 8.0
   
   ////////////////////////////////////////////////////////////////////////
   // Private implementation
   
   fileprivate static let cellIdentifier = "APUIAutocompleteTextFieldCell"
   ...
   fileprivate var filteredResults = [UIAutocompleteTextFieldItem]()
   fileprivate var filterDataSource = [UIAutocompleteTextFieldItem]() {
      didSet {
   ...
   override open func willMove(toSuperview newSuperview: UIView?) {
      ...
      self.addTarget(self, action: #selector(UIAutocompleteTextField.textFieldDidChange), for: .editingChanged)
      self.addTarget(self, action: #selector(UIAutocompleteTextField.textFieldDidBeginEditing), for: .editingDidBegin)
      self.addTarget(self, action: #selector(UIAutocompleteTextField.textFieldDidEndEditing), for: .editingDidEnd)
      self.addTarget(self, action: #selector(UIAutocompleteTextField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
      
      #if swift(>=4.2)
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
      #else
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(UIAutocompleteTextField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
   ...
   fileprivate func redrawSearchTableView() {
      ...
      if let tableView = tableView {
         ...
         if self.direction == .down {
            ...
            var tableViewFrame = CGRect(x: 0, y: 0, width: frame.size.width, height: tableHeight - 8)
            ...
            let tableHeight = min((tableView.contentSize.height), (UIScreen.main.bounds.size.height - frame.origin.y))
            UIView.animate(withDuration: 0.2, animations: { [weak self] in
   ...
   @objc open func textFieldDidChange() {
      ...
      timer?.invalidate()
      timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(UIAutocompleteTextField.typingDidStop), userInfo: self, repeats: false)
   ...
   fileprivate func highlightAttributesForSubtitle() -> [NSAttributedString.Key: AnyObject] {
      ...
      for attr in highlightAttributes {
         if attr.0 == NSAttributedString.Key.font {
            ...
            let pointSize = (attr.1 as! UIFont).pointSize
   ...
   fileprivate func prepareDrawTableResult() {
      ...
      if let keyboardFrame = keyboardFrame {
         ...
         if keyboardFrame.intersects(frame) {
            direction = .up
         } else {
            direction = .down
         }
         ...
         if self.center.y > UIApplication.shared.keyWindow!.frame.size.height {
            direction = .up
         } else {
            direction = .down
         }
...
extension UIAutocompleteTextField: UITableViewDelegate, UITableViewDataSource {
   ...
   public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
      var cell = tableView.dequeueReusableCell(withIdentifier: UIAutocompleteTextField.cellIdentifier)
      
      if cell == nil {
         cell = UITableViewCell(style: .subtitle, reuseIdentifier: UIAutocompleteTextField.cellIdentifier)
      }
...
public struct UIAutocompleteTextFieldTheme {
   ...
   public static func lightTheme() -> UIAutocompleteTextFieldTheme {
      return UIAutocompleteTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 0.6), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 12), fontColor: UIColor.black)
   }
   
   public static func darkTheme() -> UIAutocompleteTextFieldTheme {
      return UIAutocompleteTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 0.6), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 12), fontColor: UIColor.white)
   }
}
...
open class UIAutocompleteTextFieldItem {
   // Private vars
...
public typealias UIAutocompleteTextFieldItemHandler = (_ filteredResults: [UIAutocompleteTextFieldItem], _ index: Int) -> Void

public typealias UIAutocompleteTextDidChangeHandler = (UIAutocompleteTextField) -> Void
